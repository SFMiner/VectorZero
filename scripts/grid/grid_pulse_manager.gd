...
func _ready() -> void:
	...
	# Initialize tracking using global constant
...
func _check_grid_crossing() -> void:
	# Convert player position to grid coordinates using global constant
	var grid_pos = GameConstants.world_to_grid(player.global_position)
	...
	if current_grid_x != last_grid_x:
		var pulse_world_x = GameConstants.grid_to_world_scalar(current_grid_x)
	...
	if current_grid_y != last_grid_y:
		var pulse_world_y = GameConstants.grid_to_world_scalar(current_grid_y)
